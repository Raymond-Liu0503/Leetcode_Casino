...
from main import (
    LeetcodeTracker, User, SolvedProblem, InvestmentHistory, WheelSpin,
    DIFFICULTY_VALUES, DIFFICULTY_TOKENS, STREAK_MULTIPLIER, DAILY_DECAY_RATE
)
...
class TestDailyDecay:
    ...
    def test_no_decay_when_streak_active(self, tracker, user):
        ...
        user.last_solved_date = datetime.now(timezone.utc) - timedelta(days=1)
        tracker.session.commit()
    ...
    def test_decay_applies_when_no_streak(self, tracker, user):
        ...
        user.last_solved_date = datetime.now(timezone.utc) - timedelta(days=2)
        tracker.session.commit()
